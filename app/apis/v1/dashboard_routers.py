...
@router.get("/summary")
async def get_dashboard_summary(current_user: User = Depends(get_request_user)) -> Dict[str, Any]:
    """대시보드 요약 정보를 반환합니다."""
...
async def get_health_metrics(user_id: int, week_ago: datetime.date) -> Dict[str, Any]:
    """건강 지표 데이터를 가져옵니다."""
...
async def get_today_medication(user_id: int, today: datetime.date) -> Dict[str, Any]:
    """오늘의 복약 정보를 가져옵니다."""
...
async def get_lifestyle_trends(user_id: int, week_ago: datetime.date) -> Dict[str, Any]:
    """생활 변화 트렌드를 가져옵니다."""
